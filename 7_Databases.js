...
// Types of databases 
    // GraphDB = used for specific niche purposes.
...
// why we use this? = If we don't know that the statement that is written is wrong or faulty then we throw that inside try{}
//  And if error exists then it will stop the control and print whatever inside the catch{} is.
    try {
        const decoded = jwt.verify(token, jwtPassword);  // here we are checking if the jwtpassword is right 
        const username = decoded.username;   // If the password is right then give the username of the user and if NO goto Catch()
